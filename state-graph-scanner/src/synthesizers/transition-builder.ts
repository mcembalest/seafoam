...
export function buildTransitions(
  ...
  for (const entryState of entryStates) {
    ...
    const hasInitialTransition = transitions.some(
      t => t.from === 'initial' && t.to === entryState
    );
...
function findSourceStates(action: Action, states: Map<StateId, State>): StateId[] {
  const sources: StateId[] = [];
...
function findDestinationStates(action: Action, states: Map<StateId, State>): StateId[] {
  const destinations: StateId[] = [];
...
function inferDestinationState(fromState: StateId, action: Action): StateId | null {
  const actionLabels = Array.from(action.labels);
...
function findEntryStates(states: Map<StateId, State>): StateId[] {
  const entries: StateId[] = [];

  for (const [stateId, state] of states.entries()) {
    if (stateId === 'initial') continue;
