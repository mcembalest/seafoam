import { ExtractedStructure, Action, ActionId } from '../types.js';

export function extractActions(structure: ExtractedStructure): Map<ActionId, Action> {
  ...
  structure.eventHandlers.forEach((handler, idx) => {
    ...
    if (handler.event === 'click') {
      ...
      if (uiElement) {
        ...
        if (text.includes('save')) labels.add('save');
        if (text.includes('generate')) labels.add('generate');
        if (text.includes('create')) labels.add('create');
        if (text.includes('delete')) labels.add('delete');
        if (text.includes('edit')) labels.add('edit');
        if (text.includes('upload')) labels.add('upload');
        if (text.includes('submit')) labels.add('submit');
        if (text.includes('cancel')) labels.add('cancel');
        if (text.includes('close')) labels.add('close');
        if (text.includes('open')) labels.add('open');
      }
  ...
  structure.uiElements
    ...
    .forEach(button => {
      ...
      if (actions.has(actionId)) return;
  ...
  structure.apiEndpoints.forEach((endpoint, idx) => {
    ...
    if (path.includes('save')) labels.add('save data');
    if (path.includes('delete')) labels.add('delete data');
    if (path.includes('update')) labels.add('update data');
    if (path.includes('create')) labels.add('create data');
    if (path.includes('get') || endpoint.method === 'GET') labels.add('fetch data');
    if (path.includes('compose') || path.includes('generate')) labels.add('generate content');
