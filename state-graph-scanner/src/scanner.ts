...
import { ScanResult, StateActionGraph, ExecutionGuide } from './types.js';
...
function calculateStateConfidence(graph: StateActionGraph): number {
  // States with more conditions and metadata are more confident
  let totalConfidence = 0;
  for (const state of graph.states.values()) {
    let confidence = 0.5;
    if (state.conditions.length > 0) confidence += 0.2;
    if (state.metadata?.sourceLocations && state.metadata.sourceLocations.length > 0) confidence += 0.2;
    if (state.labels.size > 2) confidence += 0.1;
    totalConfidence += Math.min(confidence, 1.0);
  }
  return graph.states.size > 0 ? totalConfidence / graph.states.size : 0;
}

function calculateActionConfidence(graph: StateActionGraph): number {
  // Actions with source locations and triggers are more confident
  let totalConfidence = 0;
  for (const action of graph.actions.values()) {
    let confidence = 0.5;
    if (action.metadata?.sourceLocation) confidence += 0.3;
    if (action.metadata?.trigger) confidence += 0.2;
    totalConfidence += Math.min(confidence, 1.0);
  }
  return graph.actions.size > 0 ? totalConfidence / graph.actions.size : 0;
}

function calculateTransitionConfidence(graph: StateActionGraph): number {
