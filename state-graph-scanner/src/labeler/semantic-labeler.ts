import { StateActionGraph, State, Action } from '../types.js';
 ...
 */
export function enrichLabels(graph: StateActionGraph): StateActionGraph {
...
function enrichStateLabels(state: State): void {
  const existingLabels = Array.from(state.labels);

  // Add variations of existing labels
  const newLabels = new Set<string>(existingLabels);
...
function enrichActionLabels(action: Action): void {
...
export function generatePathDescription(
  path: Array<{ stateId: string; actionId: string }>,
